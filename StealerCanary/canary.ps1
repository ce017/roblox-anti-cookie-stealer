<#  StealerCanary - detects infostealer staging activity.
    ...
    copies of credential stores, e.g. C:\ProgramData\<10 hex chars>\ - which is
    what the 2026-09-05 incident looked like on disk.

    Detection is by FILE CONTENT, never by filename: staged copies are routinely
    renamed to random hex with no extension, so extensions tell you nothing.
...
function Log($m) {
    "$(Get-Date -f 'yyyy-MM-dd HH:mm:ss')  $m" | Out-File $Log -Append -Encoding utf8
}
...
function Invoke-Response($dir, $why) {
    ...
    # Boot-time notice - shown by Show-PendingAlert next time the canary starts
...
ACTION REQUIRED
  ...
  2. Change passwords + sign out all sessions for any account you were
     logged into (email first, then Roblox, then everything else).
  3. Evidence: $EvDir
"@
...
function Alert($dir, $why) {
    ...
    try {
        Get-ChildItem $dir -Force -EA SilentlyContinue | Select-Object Name, Length, CreationTime |
            Out-File "$ev\listing.txt" -Encoding utf8
    } catch {}
    try {
        Get-Process | Where-Object { $_.Path } | Select-Object Name, Id, Path, StartTime |
            Sort-Object StartTime -Descending | Out-File "$ev\processes.txt" -Encoding utf8
    } catch {}
    "$dir`r`n$why`r`n$(Get-Date)" | Out-File "$ev\what.txt" -Encoding utf8

    # 1b. RESPOND - killing is the only thing fast enough to beat an upload
    $null = Invoke-Response $dir $why

    # 2. SOUND - the only channel that reliably reaches you in a fullscreen game
    ...
    # 4. POPUP - message passed as a variable, never interpolated into -Command
    try {
        $w = New-Object -ComObject WScript.Shell
    ...
    if ($ShutdownOnAlert) {
        ...
        try {
            & shutdown.exe /s /f /t $ShutdownDelaySec /c "StealerCanary: suspected credential theft blocked. Shutting down to stop upload. Abort with: shutdown /a"
...
# seed: ignore hex directories that already exist
foreach ($r in $Roots) {
